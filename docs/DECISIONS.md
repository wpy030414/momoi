...
## D52：上帝化身的位置是有意义的 / 改造折叠必须两端一份实现 / 自动演算开关放内存

**日期**：2026-09-25

**背景**：Phase 3 要让上帝在世界里有位置、让 Agent 改造世界、让世界自行运转。三处各自有一个不显然的取舍。

**决策**：

1. **上帝化身的位置真的影响交互**：只有感知半径（0.6）内的存在能在简报里读到祂的确切坐标，远处的只读到「你能感到祂的存在，却无法确定祂在何处」
2. **补丁折叠只有一份实现**（`foldPatches`），网格与逐点采样共用
3. **自动演算开关是内存态**，不加 `worlds` 列，因而挂在快照旁而非 `WorldState` 里

**原因**：

- 若化身只是个装饰（放在哪都一样），「把 Avatar 放在世界的任何位置」就退化成一句空话。让距离决定「谁看得见你」是最小而有实感的落点，且它复用已有的简报机制，不新增任何概念
- 补丁折叠曾经**只有网格侧有**：Agent 抬高了地形，而服务端描述地形时仍报基准高度 —— 两边各说各话，且不报错、类型检查也看不出。这类「两份实现慢慢漂移」的缺陷是静默的，必须从结构上消除
- PG 在 `db/pg.ts` 里**没有任何 `ALTER` 通道**（只有 `CREATE TABLE IF NOT EXISTS`），给既有表加列在 PG 上是手工运维步骤。而自动演算开关：重启即关闭是**合理且安全**的默认（世界不该在主人不知情时自己跑起来），且本仓库已有 `infiniteState`（无限演算模式）的内存态先例

**备选与权衡**：

- ❌ 化身纯装饰 → 见上
- ❌ 加 `worlds.auto_tick` 列 → 付 PG 那条不存在的 ALTER 通道的代价，换一个语义上「本就该重启复位」的开关
- ❌ 上帝也可以改造世界（加一个 God 版 reshape 端点）→ 用户未要求；上帝的力量体现在**提示词**（定义世界规则）而非逐次干预。若日后需要，`world_patches.source` 已预留 `'god'` 取值
- ❌ 自动演算用 `setInterval` → 一个回合可能跑十几秒，固定间隔会让两回合重叠，而 `turn` 与实体位置都不允许交错写
- ❌ 自动演算也记一条上帝事件 → 日志会被一串一模一样的「（时间流逝）」淹掉

**影响**：

- 新增 `world_patches` 表、`lib/world-ticker.ts`、`world_reshape` 工具
- `POST /api/worlds/:id/god`、`POST /api/worlds/:id/auto-tick`
- `WorldSnapshot` 加 `patches` 与 `auto_tick`；`world_turn` 实时事件加可选的 `auto_tick`
- 改造事件的 `payload` 携带补丁本体 → 客户端就地并入，不必为一次改造重拉整个快照
- 客户端 `WorldCanvas` 有了第二条更新路径：补丁变化时**只重建高度场**，不重建渲染器与相机（否则用户正盯着的那片地会被重置视角）
