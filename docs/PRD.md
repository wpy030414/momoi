...
**上帝与造物**：用户可把上帝化身**放置在世界的任何位置**（点击沙盘任意处）——位置真的影响交互：只有近旁的存在能感知到祂在哪。**Agent 可以改造世界**（抬高/压低/凿开/削平/灌水/更换地表），改造是永久的、当场可见的、且**不改写世界地形规则**（叠加层）。还有**自动演算**开关：开启后世界每约 15 秒自行推进一拍，让用户只看不插手。

**三个阶段均已完成**：Phase 1（创生、地形生成与渲染、相机操作、法则编辑）；Phase 2（Agent 入场、回合制行动、事件日志、世界工具）；Phase 3（上帝化身与邻近感知、Agent 改造世界、自动演算）。
